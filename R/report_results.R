...
# *** NEW HELPER FUNCTION for plain number output *** (DG, 2025/12/1)
plain_format<-function(num) {
  format(num, digits=3)
}
...
#' @param histfile a string of the histogram being read by genomescope
#' @export
report_results <- function(kmer_hist,kmer_hist_orig, k, p, container, foldername, arguments, IN_VERBOSE, histfile) {
  ...
  # changed DG 2025.12.08 to show the sample name in the plot

  szSampleName <- gsub( ".histogram$", "\n\n\n", histfile )
  szSampleName <- gsub( ".*/","", szSampleName )
  
  plot(kmer_hist_orig, type="n", main=szSampleName,
...
#  }
  ...
  cex.lab=font_size, cex.axis=font_size, cex.main=font_size, cex.sub=font_size)
  #rect(0, 0, max(kmer_hist_orig[[1]])*1.1 , max(kmer_hist_orig[[2]])*1.1, col=COLOR_BGCOLOR)
  # change DG 2025.10.13 to eliminate space between bars of histogram
...
#  }
  ...
  width=10,height=6, useDingbats=FALSE )
  #png(paste(foldername, "/", arguments$name_prefix, "log_plot.png", sep=""), changed DG 2025.12.05
  #width=plot_size, height=plot_size, res=resolution)
  par(mar = c(5.1,4.1,6.1,2.1))
  plot(kmer_hist_orig, type="n", main="GenomeScope Profile\n\n\n",
  ...
  #png(paste(foldername, "/", arguments$name_prefix, "transformed_log_plot.png", sep=""), changed DG 2025.12.05
  #width=plot_size, height=plot_size, res=resolution)
  ...
  if(!is.null(model))
  {
    ...
    # *** DG MODIFICATION START: Find intersection point (k-mer coverage) ***
    cutoff_ind <- error_xcutoff_ind

    # Truncate x and pred
    x_cut <- x[1:cutoff_ind]
    pred_cut <- pred[1:cutoff_ind]
    
    # Corrected: Compare both curves on the original frequency scale (F).
    # Full Model on F scale: x_cut**(-transform_exp) * pred_cut
    # Error Curve on F scale: error_kmers
    first_peak_start_index <- which(x_cut**(-transform_exp) * pred_cut > error_kmers)
    
    if (length(first_peak_start_index) > 0) {
        # The first index where the condition is met
        first_index = first_peak_start_index[1]
        intersection_x = x_cut[first_index]
    }
    # *** DG MODIFICATION END ***
    ...
    # *** DG 2025.12.08 MODIFICATION START: Draw vertical line on Linear Plot ***
    if (intersection_x != -1) {
        abline(v=intersection_x, col="blue", lty="dotted", lwd=2)
        # --- printing the value of intersection_x next to the vertical line
        # 1. Determine the maximum Y value from the original histogram data (kmer_hist$V2).

        # new
        # Get the current Y-axis boundaries of the plot
        # par("usr") returns (x_min, x_max, y_min, y_max) of the plotting region.
        y_axis_limit <- par("usr")[4] 
        
        # Set the Y position to 95% of the plot's maximum Y limit
        y_top_pos <- y_axis_limit * 0.5
        # 3. Add the text label near the top of the plot
        text(
          x = intersection_x, 
          y = y_top_pos, 
          #labels = paste(plain_format(intersection_x), "X"),
          labels = plain_format(intersection_x),
          # Default positioning will center the text horizontally over intersection_x
          col = "blue", 
          font = 2, # Bold font
          cex = 2.0
        )
    ...
    if (NO_UNIQUE_SEQUENCE) {
      ...
      legend=c("observed", "full model", "errors", "error kmers cutoff"), # *** Added "error kmers cutoff" to legend *** DG, 2025.12.05
      #lty=c("solid", "solid", "solid", "dashed"), changed DG 2025.12.05
      lty=c("solid", "solid", "solid", "dotted"),
      lwd=c(3,3,3,2),
      #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, COLOR_KMERPEAK),
      col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, "blue"),
      bg="white")
    } else {
      legend(.62 * x_limit_orig, 1.0 * y_limit_orig,
      #legend=c("observed", "full model", "unique sequence", "errors", "kmer-peaks"), # changed DG 2025.12.05
      legend=c("observed", "full model", "errors", "error kmers cutoff"),
      #lty=c("solid", "solid", "solid", "solid", "dotted"),
      lty=c("solid", "solid", "solid", "dotted"),
      lwd=c(3,3,3,3,2),
      #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_pPEAK, COLOR_ERRORS, COLOR_KMERPEAK), DG 2025.12.05
      col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, "blue"),
      bg="white")
    }
    ...
    if (NO_UNIQUE_SEQUENCE) {
      ...
      #legend=c("observed", "full model", "errors", "kmer-peaks"), changed DG 2025.12.05
      legend=c("observed", "full model", "errors", "error kmers cutoff"),
      #lty=c("solid", "solid", "solid", "dashed"), changed DG 2025.12.05
      lty=c("solid", "solid", "solid", "dotted"),
      lwd=c(3,3,3,2),
      #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, COLOR_KMERPEAK), changed DG 2025.12.05
      col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, "blue"),
      bg="white")
    } else {
      legend(.62 * x_limit, 1.0 * y_limit,
      #legend=c("observed", "full model", "unique sequence", "errors", "kmer-peaks"), DG 2025.12.05
      legend=c("observed", "full model", "unique sequence", "errors", "error kmers cutoff"),
      #lty=c("solid", "solid", "solid", "solid", "dashed"), changed DG 2025.12.05
      lty=c("solid", "solid", "solid", "solid", "dotted"),
      lwd=c(3,3,3,3,2),
    ...
    # added DG 2025.12.01
    # *** MODIFICATION START: Draw vertical line on Log Plot ***
    if (intersection_x != -1) {
        abline(v=intersection_x, col="blue", lty="dotted", lwd=2)
    }
    # *** MODIFICATION END ***

    # added to write error and calculated to a file:
    # cutoff_ind is already defined and used above
    
    # Truncate x and pred are already done above
    # x_cut <- x[1:cutoff_ind]
    # pred_cut <- pred[1:cutoff_ind]
    ...
    data_to_write <- data.frame(
        X_Value = x_cut,
        Predicted_Value = pred_cut,
        Error_Kmers = error_kmers
    )

    # 3. Write the data frame to a file
    write.table(
      data_to_write,
        file = paste( foldername, "/plot_data.txt", sep= "" ),
        sep = "\t",
        row.names = FALSE,
        quote = FALSE # Optional: prevents surrounding text with quotes
    )

# write.csv(
#          data_to_write,
#          file = "plot_data_output.csv",
#          row.names = FALSE
#        )       
    ...
    if( !ISCROPPED )
    {
      if (NO_UNIQUE_SEQUENCE) {
        ...
        lty=c("solid", "solid", "solid", "solid", "dotted"),
        lwd=c(3,3,3,3,2),
        #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_pPEAK, COLOR_ERRORS, COLOR_KMERPEAK), changed DG 2025.12.05
        col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_pPEAK, COLOR_ERRORS, "blue"),
        bg="white")
      }
    }
    else
    {
      if (NO_UNIQUE_SEQUENCE) {
        legend("topright",
        ##legend(exp(.62 * log(max(x))), 1.0 * max(y),
        #legend=c("observed", "full model", "errors", "kmer-peaks","cov-threshold"), changed DG 2025.12.04
        legend=c("observed", "full model", "errors", "error kmers cutoff","cov-threshold"),
        #lty=c("solid", "solid", "solid", "dashed", "dashed"), changed DG 2025.12.04
        lty=c("solid", "solid", "solid", "dotted", "dashed"),
        lwd=c(3,3,3,2,3),
        #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, COLOR_KMERPEAK, COLOR_COVTHRES), changed DG 2025.12.05
        col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_ERRORS, "blue", COLOR_COVTHRES),
        bg="white")
      } else {
        legend("topright",
        ##legend(exp(.62 * log(max(x))), 1.0 * max(y),
        #legend=c("observed", "full model", "unique sequence", "errors", "kmer-peaks","cov-threshold"), changed DG 2025.12.05
        legend=c("observed", "full model", "unique sequence", "errors", "error kmers cutoff","cov-threshold"),
        #lty=c("solid", "solid", "solid", "solid", "dashed", "dashed"), changed DG 2025.12.05
        lty=c("solid", "solid", "solid", "solid", "dotted", "dashed"),
        lwd=c(3,3,3,3,2,3),
        #col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_pPEAK, COLOR_ERRORS, COLOR_KMERPEAK, COLOR_COVTHRES), DG 2025.12.05
        col=c(COLOR_HIST, COLOR_2pPEAK, COLOR_pPEAK, COLOR_ERRORS, "blue", COLOR_COVTHRES),
        bg="white")
      }
    }
  ...
  if (p<=2 & FITTED_HIST){
	  ...
	  #png(paste(foldername, "/fitted_hist.png", sep=""), height = plot_size, width = plot_size, res=resolution) changed DG 2025.12.05
