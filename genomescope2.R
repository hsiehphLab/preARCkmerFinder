...
# --- added to be able to make changes without reinstalling the entire package (35 minutes) DG 2025.12.04
# Assuming your execution starts from the main project directory, 
# the path to the file is R/report_results.R
source("R/report_results.R")

# Verify the function signature
if (!"histfile" %in% names(formals(report_results))) {
  stop("Error: report_results function does not have histfile parameter. Source file may not have loaded correctly.")
}
# -----------------------------------------------
...
if (is.null(arguments$input) | is.null(arguments$output)) {
  cat("USAGE: genomescope2.R -i input_histogram_file -o output_dir -p ploidy -k kmer_length\n")
  ...
  while(round < NUM_ROUNDS) {
    ...
    if (!is.null(model_peaks[[1]])) {
      ...
      if (VERBOSE) {
        ...
        report_results(kmer_prof,kmer_prof_orig, k, p, model_peaks, mdir, arguments, TRUE, histfile)
      }
    }
    else {
  ...
  report_results(kmer_prof,kmer_prof_orig, k, p, best_container, foldername, arguments, FALSE, histfile)
#  if (!is.null(best_container[[1]])) {
#    print('model score')
