...
# *** NEW HELPER FUNCTION for plain number output ***
...
report_results<-function(kmer_hist,kmer_hist_orig, k, container, foldername)
{
    ...
    # *** DG NEW: Initialize Intersection X value ***
    intersection_x = -1
    # *** DG END NEW ***
    ...
    # changed png to pdf (DG, 2025.10.06)
	#png(paste(foldername, "/plot.png", sep=""),width=plot_size,height=plot_size, res=resolution)
	pdf(paste(foldername, "/plot.pdf", sep=""),width=10,height=6, useDingbats=FALSE )
    # end change
    ...
    # make background white (DG, 2025.10.13)
    ...
    #points(kmer_hist_orig, type="h", col=COLOR_HIST, lwd=7)
    ...
    # changed from png to pdf (DG, 2025.10.06)    
	pdf(paste(foldername, "/plot.log.pdf", sep=""),width=10,height=6, useDingbats=FALSE )
	...
	if(!is.null(container[[1]])) 
    {
       ...
       # Find the first index where the predicted model (pred_cut) is greater than the error signal (error_kmers)
       first_peak_start_index <- which(pred_cut > error_kmers) 
...
#        )       
       ...
       if(length(kmer_hist[,1])==length(kmer_hist_orig[,1])){
           legend(exp(.65 * log(max(x))), 1.0 * max(y),
              ...
              legend=c("observed", "full model", "errors", "error kmers cutoff"), # *** MODIFICATION: Added "error kmers cutoff" to legend ***
              #legend=c("observed", "full model", "unique sequence", "errors", "kmer-peaks"),
              #lty=c("solid", "solid", "solid", "solid", "dashed"),
              lty=c("solid", "solid", "solid", "dotted"), # *** MODIFICATION: Added "dotted" line type ***
              lwd=c(3,3,3,2), # *** MODIFICATION: Added line width for new line ***
              # removed some info from plots (DG, 2025.10.06)
              col=c(COLOR_HIST, COLOR_4PEAK, COLOR_ERRORS, "blue"), # *** MODIFICATION: Added "blue" color ***
       ...
       # *** DG MODIFICATION START: Draw vertical line on Linear Plot ***
       if (intersection_x != -1) {
           abline(v=intersection_x, col="blue", lty="dotted", lwd=2)
       }
       # *** DG MODIFICATION END ***
...
if(length(args) < 4) {
    ...
    # site of current error
    print( paste( "k =", k ) )

    print( paste("readlength =", readlength) )
    ...
    # reverted to original DG, 2025.10.31 to use tabs instead of spaces
